...
/// Build the Content-Security-Policy. The dashboard fans in JSON only (no
/// third-party images/media), so everything stays same-origin except the Google
/// Fonts hosts halo-design uses. HSTS / X-Frame-Options / X-Content-Type-Options
/// are Traefik's job, not the binary's.
///
