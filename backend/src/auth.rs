...
/// The authenticated principal, extracted from the forward-auth headers. Present
/// on every `/api/*` handler purely as a gate — the dashboard is read-only and
/// does nothing per-user with the identity.
#[derive(Debug, Clone)]
pub struct AuthUser {
    pub user: String,
    pub email: String,
