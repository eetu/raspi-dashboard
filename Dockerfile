...
RUN mkdir -p backend/src e2e/src \
    ...
    && : > backend/src/lib.rs \
    && : > e2e/src/lib.rs \
    && xx-cargo build --release -p raspi-dashboard-backend
...
WORKDIR /app
LABEL org.opencontainers.image.description="raspi-dashboard — service health + metrics + CVE status for the Pi"
LABEL org.opencontainers.image.source="https://github.com/eetu/raspi-dashboard"

COPY --from=backend-build /etc/ssl/certs/ca-certificates.crt /etc/ssl/certs/
COPY --from=backend-build /app/target/*/release/raspi-dashboard-backend ./raspi-dashboard-backend
COPY --from=frontend-build /app/dist ./dist

ENV STATIC_DIR=./dist
ENV DASHBOARD_BIND=0.0.0.0:3007

USER 1000

EXPOSE 3007

CMD ["./raspi-dashboard-backend"]
